...
function startGame() {
    ...
    document.getElementById("score").innerHTML = "Puntos: 0";
...
function draw() {
    ...
    // FRUTA
    ctx.beginPath();
    ctx.arc(food.x, food.y, 9, 0, Math.PI * 2);
    ctx.fillStyle = "red";
    ctx.fill();
    ...
    snake.forEach((part, index) => {
        ctx.beginPath();
        let radio = index === 0 ? 10 : 8 - index * 0.1;
        ctx.arc(part.x, part.y, radio, 0, Math.PI * 2);
...
function gameOver() {
    ...
    document.getElementById("finalScore").innerHTML = "Puntos alcanzados: " + score;
}
